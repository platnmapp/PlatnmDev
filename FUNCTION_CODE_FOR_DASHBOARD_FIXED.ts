...
// Use this version - it has the correct imports for Supabase dashboard

// Note: Removed Supabase client import - not needed since we're not using database caching
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get a fresh Spotify token (no database caching needed for now)
    // Tokens are valid for 1 hour, so we'll fetch a new one each time
    const tokenResult = await getNewSpotifyToken();
